<%@ Control Language="C#" AutoEventWireup="true" CodeFile="mod_category_product.ascx.cs" Inherits="admincp_modules_mod_product_mod_category_product" %>
		...
		<tr>
			...
			<tr>
				...
				<tr>
					...
					-->					
					<%= clsHtml.add(2, "Default.aspx?page=add_edit_category_product&mod=product&do=add")%>
					</td>
				...
				<table class="content">
					...
					<%	
                        ...
                        //------------------------------ 
                        string strSqlCount = "select count(*) from tbl_category_product where FK_ParentID = 0 AND FK_LangID = " + lang.getLangID();
                        ...
                        //========================================		    		                             	    	               			        		    
                        string strSql = "select * from tbl_category_product where FK_ParentID = 0 AND FK_LangID = " + lang.getLangID() + " order by C_Rank desc";
                        DataTable dt;
                        ...
                        if (dt.Rows.Count > 0)
                        {
					     ...
					     %>
							<tr class="light" >								
								...
								<td align="center" >
                                    <% if (clsSwap.getMaxRankRecord("tbl_category_product", "FK_ParentID = 0 and FK_LangID =" + lang.getLangID()) > (int)dt.Rows[i]["C_Rank"])
                                    {
                                %>
                                ...
                                %>
								<% if (clsSwap.getMinRankRecord("tbl_category_product", "FK_ParentID = 0 and FK_LangID =" + lang.getLangID()) < (int)dt.Rows[i]["C_Rank"])
                                    {
                                %>
								...
								<% } %>									
								&nbsp;							
								<%= clsHtml.edit(2, "Default.aspx?page=add_edit_category_product&mod=product&do=edit&id=" + dt.Rows[i]["PK_CategoryID"].ToString())%> 
								&nbsp;								
								<%= clsHtml.del(2, clsConfig.getCurrentUrl() + "&do=delete&id=" + dt.Rows[i]["PK_CategoryID"].ToString())%>	
					                    ...
					                    <%	
                                    string strSqlSub = "select * from tbl_category_product where FK_ParentID = "+ dt.Rows[i]["PK_CategoryID"].ToString() +" AND FK_LangID = " + lang.getLangID() + " order by C_Rank desc";
                                    DataTable dtSub;
                                    ...
                                    if (dtSub.Rows.Count > 0)
                                    {
			                         ...
			                         %>
					                    <tr class="light" >								
            								...
            								<% if (clsSwap.getMaxRankRecord("tbl_category_product", "FK_ParentID = " + dt.Rows[i]["PK_CategoryID"].ToString() + " and FK_LangID =" + lang.getLangID()) > (int)dtSub.Rows[j]["C_Rank"])
                                                {
                                            %>
                                            ...
                                            %>
						                    <% if (clsSwap.getMinRankRecord("tbl_category_product", "FK_ParentID = " + dt.Rows[i]["PK_CategoryID"].ToString() + " and FK_LangID =" + lang.getLangID()) < (int)dtSub.Rows[j]["C_Rank"])
                                                {
                                            %>
						                    ...
						                    <% } %>									
						                    &nbsp;							
						                    <%= clsHtml.edit(2, "Default.aspx?page=add_edit_category_product&mod=product&do=edit&id=" + dtSub.Rows[j]["PK_CategoryID"].ToString())%> 
						                    &nbsp;								
