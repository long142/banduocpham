...
/*
    ...
    */
    ...
    field.action = "Default.aspx?page=product&mod=product&do=search&key=" + field.key.value +"&code=" + field.code.value + "&per=" + field.per.value + "&cat=" + field.ctl03$cat.value;
    field.submit();
    return true;
}
...
<%
    ...
    if (intCat != 0)
        strSearchCat = " and (tbl_category_product.PK_CategoryID = " + intCat + " or tbl_category_product.FK_ParentID = " + intCat + ")";

    string strKey = clsInput.getStringInput("key", 0);
    string strCode = clsInput.getStringInput("code", 0);
    string strSearch = "";
    if (strKey != null)
        strSearch = " and (tbl_product.C_Name like '%" + strKey + "%' )";
    if (strCode != null)
        strSearch = " and (tbl_product.C_Code like '%" + strCode + "%' )";
        
    //------------------------------ 		    
    string strSqlCount = "select count(*) from tbl_product inner join tbl_category_product on tbl_product.FK_CategoryID = tbl_category_product.PK_CategoryID where tbl_product.FK_LangID = " + lang.getLangID() + strSearch + strSearchCat;
    ...
    //========================================		    		                             	    	               			        		    
    string strSql = "select tbl_product.*, tbl_category_product.C_Name as CategoryName from tbl_product inner join tbl_category_product on tbl_product.FK_CategoryID = tbl_category_product.PK_CategoryID where tbl_product.FK_LangID = " + lang.getLangID() + strSearch + strSearchCat + " order by tbl_product.C_Rank desc";
    DataTable dt;
									...
									<table width="100%" cellpadding="0" cellspacing="0" border="0">
										<tr>
											<th align="left" valign="middle" width="100%">
                                                ...
                                                Mã sản phẩm :&nbsp;<input type="text" name="code" style="width:200px" value="<%=strCode %>"/>&nbsp;
					...
					<%	
                        ...
                        if (dt.Rows.Count > 0)
                        {
                            ...
                            for (int i = 0; i < dt.Rows.Count; i++)
                            {
                                ...
                                DataTable dsParentCategory = clsDatabase.getDataTable("Select C_Name from tbl_category_product where PK_CategoryID =(select FK_ParentID from tbl_category_product where PK_CategoryID=" + dt.Rows[i]["FK_CategoryID"].ToString() + ")");
                                if (dsParentCategory.Rows.Count > 0)
                                    parentCategoryName = dsParentCategory.Rows[0][0].ToString();
					     ...
					     %>
							<tr class="light">
								...
								<td align="Left">														
								&nbsp;
								<%=parentCategoryName%> - <%= dt.Rows[i]["CategoryName"].ToString()%>
								</td>		
								<td align="center">
